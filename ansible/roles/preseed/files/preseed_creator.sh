...
if [[ -z $INPUT ]]
then
    INPUT=$(wget -q -O - https://cdimage.debian.org/debian-cd/current/amd64/iso-cd/ | grep "netinst.iso" | grep -v "debian-mac" | grep -v "debian-edu" | sed -e 's@.*a href="\([^"]*\)".*@\1@')
    ...
    then
        ...
        wget -q https://cdimage.debian.org/debian-cd/current/amd64/iso-cd/$INPUT -O $INPUT
    fi
